...
for (const element of document.getElementsByTagName("a")) {
    if (element.getAttribute("type") == "social-icon") {
        openMouseWithTooltop(element, 70);
    }
...
function openMouseWithTooltop(element, width) {
    element.addEventListener("mouseover", () => {
        mouse.style.opacity = "0";
        element.querySelector('svg').setAttribute("fill", element.getAttribute("fillOnHover") ?? "#fff");
...
hideMouseOnHover(document.getElementById("projects-btn"));
hideMouseOnHover(document.getElementById("skills-btn"));
